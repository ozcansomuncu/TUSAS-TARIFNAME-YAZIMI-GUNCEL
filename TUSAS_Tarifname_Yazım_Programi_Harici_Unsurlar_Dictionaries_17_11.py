# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 14:29:10 2021
...
Label_1= Label(root, text="Toplam Harici Unsur Sayısını Giriniz:") #.grid(row=0, column=1) (Alternatif yöntem)
label_1_window = my_canvas.create_window(35, 10, anchor="nw", window= Label_1) 

# Toplam Unsur Sayısı unsurunu depola

toplam_harici_unsur_sayisi_intvar = IntVar()

toplam_harici_unsur_sayisi_entry = Entry(root, textvariable=toplam_harici_unsur_sayisi_intvar)

toplam_harici_unsur_sayisi_entry_window= my_canvas.create_window(60, 30, anchor="nw", window= toplam_harici_unsur_sayisi_entry) 

#toplam_harici_unsur_sayisi_entry.grid(row=1, column=1)

def show_entry_fields(): #Bu fonksiyon, GUI'de girilen unsur sayısını kullanıcıya göstermek içindir.
    girilen_unsur_sayisi_text = my_canvas.create_text(250, 250, text= "Girilen harici unsur sayisi: 0", font= ("Helvetica", 20), fill="black")
    my_canvas.delete(girilen_unsur_sayisi_text)
    girilen_unsur_sayisi_text = my_canvas.create_text(250, 250, text= "Girilen harici unsur sayisi: %s" % (toplam_harici_unsur_sayisi_entry.get()), font= ("Helvetica", 20), fill="black")
    print("Girilen harici unsur sayisi: %s" % (toplam_harici_unsur_sayisi_entry.get())) #Bu kodu eğer "konsolda da" yazdırmak istiyorsan kullanabilirsin. 
    #Ama zaten GUI'de yazdığı için buna gerek yok.

Button_1 = Button(root, text='Harici Unsur Sayisini Göster', command=show_entry_fields) #.grid(row=3, column=0, sticky=W, pady=4)
...
def open():   
    ...
    #global toplam_harici_unsur_sayisi, my_entries_unsur_isimleri, my_entries_unsur_tanimlari, unsur_isimleri_listesi,unsur_tanimlari_listesi, final_istem_cümlesi
    #problem globalden dolayı değilmiş. Şuan problem 1. GUI'de tuşa basınca 2. fonksiyonun otomatik çalışması!
    toplam_harici_unsur_sayisi = toplam_harici_unsur_sayisi_intvar.get()

    for i in range (1,toplam_harici_unsur_sayisi+1):
        harici_unsur_sayilari_listesi.append (i)
    ...
    for z in range (1,toplam_harici_unsur_sayisi+1):
    ...
    for x in range (1, toplam_harici_unsur_sayisi+1):  
    ...
    for x in range (1, toplam_harici_unsur_sayisi+1):  
       ...
       my_entries_unsur_isimleri_entry.grid(row=x, column=2, pady=20, padx=5)
       my_entries_unsur_isimleri.append(my_entries_unsur_isimleri_entry)
        
    #Harici Unsur Tanımları Sütunu - Row Loop
    for y in range (1, toplam_harici_unsur_sayisi+1):  
       my_entries_unsur_tanimlari_entry = Entry (master)
       my_entries_unsur_tanimlari_entry.grid(row=y, column=3, pady=20, padx=5, ipadx=50, ipady=10)
    ...
    def show_values ():
        ...
        for q in range(toplam_harici_unsur_sayisi):
        ...
        for w in range(toplam_harici_unsur_sayisi-1):
        ...
        harici_unsur_referanslari_listesi = StringVar ()
        
        harici_unsur_referanslari_listesi = []
        #Yukarıda unsur isimleri listesini oluşturmak için boş bir liste atadık. Daha sonra buraya unsur isimlerini atayacağız.
            
        for i in range (0, toplam_harici_unsur_sayisi):
            harici_unsur_referanslari_listesi.append (i)
        ...
        for i in range (0, toplam_harici_unsur_sayisi):
            harici_unsur_referanslari_listesi [i] = my_entries_unsur_referanslari [i].get()
            
            #Yukarıda, my_entries listesinden değerleri unsur referanslarını atadık.    
               
        print ('Harici Unsur Referansları Sırası ile aşağıdaki gibidir: ')
        print (harici_unsur_referanslari_listesi)
        
        ###------------------------------------------------------------------------------###
        harici_unsur_isimleri_listesi = StringVar ()
        
        harici_unsur_isimleri_listesi = []
            
        #Yukarıda, unsur tanımları için boş liste tanımlanmışır.
            
        for i in range (0, toplam_harici_unsur_sayisi):
            harici_unsur_isimleri_listesi.append (i)
        ...
        for i in range (0, (toplam_harici_unsur_sayisi)):    
            harici_unsur_isimleri_listesi [i] = my_entries_unsur_isimleri [i].get()
                  
            #Yukarıdaki for döngüsü ile 1. indexten n. idexe kadar, yani 2. unsurdan n. unsura kadar unsur tanımlarını aldık.
            
        print ('Harici Unsur Tanımları Sırası ile aşağıdaki gibidir: ')
        print (harici_unsur_isimleri_listesi)
        
        ###-----------------------------------------------------------------------------------###
       
        harici_unsur_tanimlari_listesi = StringVar ()
       
        harici_unsur_tanimlari_listesi = []
            
        #Yukarıda, unsur tanımları için boş liste tanımlanmışır.
            
        for i in range (0, toplam_harici_unsur_sayisi):
            harici_unsur_tanimlari_listesi.append (i)
            
        #unsur_sayilari_listesi ile aynı üyeleri olan listeyi oluşturduk ki boş listemizde aynı sayıda unsur tanımları olsun. 
        #Daha sonra gerçek unsur tanımlarını gireceğiz.
            
        for i in range (0, (toplam_harici_unsur_sayisi)):    
            harici_unsur_tanimlari_listesi [i] = my_entries_unsur_tanimlari [i].get()
                  
            #Yukarıdaki for döngüsü ile 1. indexten n. idexe kadar, yani 2. unsurdan n. unsura kadar unsur tanımlarını aldık.
            
        print ('Harici Unsur Tanımları Sırası ile aşağıdaki gibidir: ')
        print (harici_unsur_tanimlari_listesi)
        ...
        harici_unsurlar = StringVar ()
        
        harici_unsurlar = []
        
        for i in range (toplam_harici_unsur_sayisi):
            harici_unsurlar.append(toplam_harici_unsur_sayisi)
        ...
        for i in range (toplam_harici_unsur_sayisi):
            harici_unsurlar [i] = {
                referans: harici_unsur_referanslari_listesi [i],
                isim: harici_unsur_isimleri_listesi [i],
                tanim: harici_unsur_tanimlari_listesi [i]
        }
        
        for i in range (toplam_harici_unsur_sayisi):
            print (harici_unsurlar [i].get(referans))
            print (harici_unsurlar [i].get(isim))
            print (harici_unsurlar [i].get(tanim))
        ...
        for i in range (0, toplam_harici_unsur_sayisi):    
            harici_unsur_tanimlari_listesi [i] = str(harici_unsurlar [i].get(tanim))
                  
            #Yukarıdaki for döngüsü ile unsur tanımlarını aldık.
        ...
        harici_unsur_tanimlari_listesi_split = []
        ...
        for i in range (0, toplam_harici_unsur_sayisi):
            harici_unsur_tanimlari_listesi_split.append (i)
            
        for i in range (0,toplam_harici_unsur_sayisi):
            harici_unsur_tanimlari_listesi_split [i] = harici_unsur_tanimlari_listesi [i].split ()
        
        ### harici_unsurlar [i].get(tanim)
        
        #unsur_referanslari_listesi = ["u1","u2","u3","u4","u5","u6","u7","u8","u9","u10","u11","u12","u13","u14","u15","u16","u17","u18","u19","u20"]
            
        for i in range (0, toplam_harici_unsur_sayisi):
           for l, item_3 in enumerate (harici_unsur_tanimlari_listesi_split [i]):
              if harici_unsur_tanimlari_listesi_split [i] [l].startswith (harici_unsurlar [i].get(isim)):
                 harici_unsur_tanimlari_listesi_split [i] [l] = item_3 + (harici_unsurlar [i].get(referans))
              else:
                 harici_unsur_tanimlari_listesi_split [i] [l] = item_3
        ...
        harici_unsur_tanimlari_listesi_string = []
            
        for i in range (0,toplam_harici_unsur_sayisi):
            harici_unsur_tanimlari_listesi_string.append(harici_unsur_tanimlari_listesi_split [i])
        ...
        for i in range (0, toplam_harici_unsur_sayisi):
           harici_unsur_tanimlari_listesi_string [i] = ' '.join(harici_unsur_tanimlari_listesi_split [i])
            
        #- REPLACE FONKSİYONUNU GERÇEKLEŞTİREBİLMEK İÇİN TÜM İFADEYİ LİST'TEN STRING FORMATINA GERİ ÇEVİRDİK JOİN FONKSİYONU İLE.
        
        for i in range (toplam_harici_unsur_sayisi):
            harici_unsurlar [i].get(tanim) == harici_unsur_tanimlari_listesi_string [i]
        ...
        for i in range (0,toplam_harici_unsur_sayisi):
            if i <= toplam_harici_unsur_sayisi:
                istem_cümlesi [i]=  harici_unsurlar [i].get(tanim) + ' en az bir ' + harici_unsurlar [i].get(isim) + ' (' + ((harici_unsurlar [i].get(referans)).capitalize()) + '), '
            else:
                break
...
'''
    ...
    '''
'''
    text_editor_button = Button (master, text="Text Editore Geç", command=master.text_editor) 
    text_editor_button.grid(row=15, column=1, pady=20)
    '''
'''
    master.mainloop()

Button_2 = Button(root, text='Harici Unsur Referansları, İsimleri ve Tanımlamalarına Geç', command=open) #command=root.quit  #.grid(row=3, column=1, sticky=W, pady=4)
...
harici_unsur_tanimlari_listesi =["hava aracından hedefine taarruz etmek amacıyla gönderilen", "Ma basınç uygulayarak stabilitesini sağlayan", "teknisyen tarafından Pları döndürmek için kullanılan"]
...
harici_unsurlar [0] = {
   referans: "M",
   isim: "mühimmat",
   tanim: "hava aracından hedefine taarruz etmek amacıyla gönderilen"
   }

harici_unsurlar [1] = {
   referans: "P",
   isim: "pabuç",
   tanim: "mühimmata basınç uygulayarak stabilitesini sağlayan"
   }

harici_unsurlar [2] = {
   referans: "A",
   isim: "alyan",
   tanim: "teknisyen tarafından pabuçları döndürmek için kullanılan"
   }

print (harici_unsurlar)
...
for i in range (0,toplam_harici_unsur_sayisi):
   unsur_isimleri_listesi_unsuz_yumusamasi_hali.append(harici_unsurlar [i].get(isim))
...
for i in range (1, toplam_harici_unsur_sayisi):
...
for i in range (0, toplam_harici_unsur_sayisi):
   for l, item_3 in enumerate (harici_unsur_tanimlari_listesi_split [i]):
       print (harici_unsur_tanimlari_listesi_split [i] [l])
'''
...
for i in range (1, toplam_harici_unsur_sayisi):
   for l, item_3 in enumerate (harici_unsur_tanimlari_listesi_split [i]):
      for k in range (0, toplam_harici_unsur_sayisi):
         if harici_unsur_tanimlari_listesi_split [i] [l].startswith (harici_unsurlar [k].get(referans)):
            harici_unsur_tanimlari_listesi_split [i] [l] = item_3 + ' (' + (harici_unsurlar [k].get(referans)) + ')'
         else:
            continue
...
harici_unsur_tanimlari_listesi_split_son = []


for k in range (0, len(harici_unsur_tanimlari_listesi_split)):
   harici_unsur_tanimlari_listesi_split_son.append(harici_unsur_tanimlari_listesi_split [k])
...
for k in range (0,toplam_harici_unsur_sayisi):
   for j, item_2 in enumerate(unsuz_yumusamasi_harfleri_listesi_initial):
       if harici_unsurlar [k].get(isim) [-1].endswith (item_2):
          print (" '{}' unsuru '{}'".format(harici_unsurlar [k].get(isim), item_2) + ' harfi ile bitmektedir ve ünsüz yumuşaması gerekebilir.') 
       else:
          continue
...
for z in range (0,toplam_harici_unsur_sayisi):
    for k in range (0, len(harici_unsur_tanimlari_listesi_split)):
       for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split [k]):
          for n in range (0,8): #sesli_harflerin toplam adedinden dolayı 8.
             if harici_unsur_tanimlari_listesi_split [k] [l] [0] == harici_unsurlar [z].get(referans) and harici_unsur_tanimlari_listesi_split [k] [l] [1] == sesli_harfler [n]:
...
for z in range (0,toplam_harici_unsur_sayisi):            
    for i, item in enumerate(harici_unsurlar [z].get(isim)):
        for j, item_2 in enumerate(unsuz_yumusamasi_harfleri_listesi_initial):
            for k in range (1, len(harici_unsur_tanimlari_listesi_split_son)):
                for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split_son [k]):
                    for n in range (0,8):   #sesli_harflerin toplam adedinden dolayı 8.             
                        if harici_unsurlar [z].get(isim) [i] [-1].endswith (unsuz_yumusamasi_harfleri_listesi_initial [j]): #Buradan 3.(i=2) ve 5. (i=4) unsur isimleri geçecek
                            if harici_unsur_tanimlari_listesi_split [k] [l].startswith (harici_unsurlar [z].get(referans)): # [k] [l] = [3] [5] ve [3] [9] geçti.
                                if harici_unsur_tanimlari_listesi_split [k] [l] [2] == sesli_harfler [n]:
                                    harici_unsur_tanimlari_listesi_split_son [k] [l] = harici_unsur_tanimlari_listesi_split_son [k] [l].replace (harici_unsurlar [z].get(referans), unsur_isimleri_listesi_unsuz_yumusamasi_hali [i])
                                else:
                                    continue
                            else: 
                                continue
...
'''            
for i, item in enumerate(harici_unsurlar [i].get(isim)):
    for k in range (1, len(harici_unsur_tanimlari_listesi_split_son)):
        for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split_son [k]):
            if harici_unsur_tanimlari_listesi_split [k] [l].startswith (harici_unsurlar [i].get(referans)): # [k] [l] = [3] [5] ve [3] [9] geçti.
                harici_unsur_tanimlari_listesi_split_son [k] [l] = harici_unsur_tanimlari_listesi_split_son [k] [l].replace (harici_unsurlar [i].get(referans), harici_unsurlar [i].get(isim) [i])
            else:
                continue
...
for i in range (1, toplam_harici_unsur_sayisi):
   harici_unsur_tanimlari_listesi_string [i] = ' '.join(harici_unsur_tanimlari_listesi_split_son [i])
...
for i in range (1,toplam_harici_unsur_sayisi+1):
   if i < toplam_harici_unsur_sayisi:
      istem_cümlesi [i]=  harici_unsur_tanimlari_listesi_string [i] + ' en az bir ' + harici_unsurlar [i].get(isim) [i] + ' (' + str(i+1) + '), '
   elif i+1 == toplam_harici_unsur_sayisi:
      istem_cümlesi [i]=  harici_unsur_tanimlari_listesi_string [i] + ' en az bir ' + harici_unsurlar [i].get(isim) [i] + ' (' + str(i+1) + ') '
   else:
      break
