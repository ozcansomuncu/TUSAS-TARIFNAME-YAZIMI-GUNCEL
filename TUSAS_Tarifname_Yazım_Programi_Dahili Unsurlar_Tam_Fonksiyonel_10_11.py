...
'''
#Alternatif metodlar (Ama bizim yöntem kadar iyi değiller, drawbackları var)

#Label oluşturuyoruz.
bg_label = Label(root, image=bg)
bg_label.place(x=0, y=0, relwidth=1, relheight=1) #Bu yöntemde kutu arkaplanları falan kalıyor, o yüzden feasible değil.

#Background Image üzerine bir şeyler koyacağız.

patentlendin_label = Label(root, text="PATENTLENDİN!", font=("Helvetica", 100), fg="black")
patentlendin_label.grid(row=5, column=5)
'''

Label_1= Label(root, text="Toplam Dahili Unsur Sayısını Giriniz:") #.grid(row=0, column=1) (Alternatif yöntem)
...
def show_entry_fields(): #Bu fonksiyon, GUI'de girilen unsur sayısını kullanıcıya göstermek içindir.
    girilen_unsur_sayisi_text = my_canvas.create_text(250, 250, text= "Girilen unsur sayisi: 0", font= ("Helvetica", 20), fill="black")
    my_canvas.delete(girilen_unsur_sayisi_text)
    girilen_unsur_sayisi_text = my_canvas.create_text(250, 250, text= "Girilen unsur sayisi: %s" % (toplam_unsur_sayisi_entry.get()), font= ("Helvetica", 20), fill="black")
    print("Girilen unsur sayisi: %s" % (toplam_unsur_sayisi_entry.get())) #Bu kodu eğer "konsolda da" yazdırmak istiyorsan kullanabilirsin. 
    #Ama zaten GUI'de yazdığı için buna gerek yok.

Button_1 = Button(root, text='Unsur Sayisini Göster', command=show_entry_fields) #.grid(row=3, column=0, sticky=W, pady=4)
...
def open():   
    ...
    for x in range (1, toplam_unsur_sayisi+1):  
       my_entry_unsur_isimleri = Entry (master)
       my_entry_unsur_isimleri.grid(row=x, column=2, pady=20, padx=5)
       my_entries_unsur_isimleri.append(my_entry_unsur_isimleri)
        
    #Harici Unsur Tanımları Sütunu - Row Loop
    for y in range (1, toplam_unsur_sayisi+1):  
       my_entry_unsur_tanimlari = Entry (master)
       my_entry_unsur_tanimlari.grid(row=y, column=3, pady=20, padx=5, ipadx=50, ipady=10)
       my_entries_unsur_tanimlari.append(my_entry_unsur_tanimlari)
       '''
       scroll = master.Scrollbar(my_entry_unsur_tanimlari)
       my_entry_unsur_tanimlari.configure(yscrollcommand=scroll.set)
       scroll.config(command=my_entry_unsur_tanimlari.yview)
       scroll.pack(side=master.RIGHT, fill=master.Y)
       '''
    ...
    my_label.grid(row=1, column=3, pady=20)
...
Button_2 = Button(root, text='Unsur İsimleri ve Unsur Tanımlamalarına Geç', command=open) #command=root.quit  #.grid(row=3, column=1, sticky=W, pady=4)

Button_2_window= my_canvas.create_window(175, 50, anchor="nw", window= Button_2) 
