...
for k in range (0, len(harici_unsur_tanimlari_listesi_split)):
    for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split [k]):
        for n in range (0,8): #sesli_harflerin toplam adedinden dolayı 8.
            if harici_unsur_tanimlari_listesi_split [k] [l] [1].isdigit() and harici_unsur_tanimlari_listesi_split [k] [l] [2] == sesli_harfler [n]:
    ...
    for j, item_2 in enumerate(unsuz_yumusamasi_harfleri_listesi_initial):
        for k in range (1, len(harici_unsur_tanimlari_listesi_split_son)):
            for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split_son [k]):
                for n in range (0,8):                
                    if harici_unsurlar [i].get(isim) [i] [-1].endswith (unsuz_yumusamasi_harfleri_listesi_initial [j]): #Buradan 3.(i=2) ve 5. (i=4) unsur isimleri geçecek
                        if harici_unsur_tanimlari_listesi_split [k] [l].startswith (harici_unsurlar [i].get(referans)): # [k] [l] = [3] [5] ve [3] [9] geçti.
                            if harici_unsur_tanimlari_listesi_split [k] [l] [2] == sesli_harfler [n]:
                                harici_unsur_tanimlari_listesi_split_son [k] [l] = harici_unsur_tanimlari_listesi_split_son [k] [l].replace (harici_unsurlar [i].get(referans), unsur_isimleri_listesi_unsuz_yumusamasi_hali [i])
                            else:
                                continue
                        else: 
                            continue
