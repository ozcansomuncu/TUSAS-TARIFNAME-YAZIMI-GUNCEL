...
def open():   
    ...
    def show_values ():
        ...
        for i in range (0, toplam_unsur_sayisi-1):
           for k in range (0, toplam_unsur_sayisi-1):
               for l, item_3 in enumerate (unsur_tanimlari_listesi_split [k]):
                  if unsur_tanimlari_listesi_split [k] [l].startswith (unsurlar [i].get(referans)):
                     unsur_tanimlari_listesi_split [k] [l] = item_3 + ' (' + str(((unsurlar [i].get(referans)).capitalize())) + '), '
                  else:
                     unsur_tanimlari_listesi_split [k] [l] = item_3 
            
        #-YUKARIDAKİ KOD İLE UNSUR TANIMLARININ SPLIT VERSIYONU LISTESİNE YANİ UNSURLARA PARANTEZLİ REFERANS NUMARALARI ATANDI.
        
        #Buradan aşağıdaki kodlar önceki hazır kodlardan direkt geldi ve ünsüz yumuşaması fonksiyonu ile ilgilidir.
        ...
        for i in range (0, toplam_unsur_sayisi-1):
            for k in range (0, len(unsur_tanimlari_listesi_split)):
                for l, item_3 in enumerate(unsur_tanimlari_listesi_split [k]):
                    for n in range (0,8):
                        if unsur_tanimlari_listesi_split [k] [l] [0] == unsur_referanslari_listesi [i]:
                            if unsur_tanimlari_listesi_split [k] [l] [1] == sesli_harfler [n]:
                                print ("Unsur tanımlarında '[{}] [{}]' indexinde olan '{}' kelimesi,'{}' referansına ve '{}' ismine sahiptir. Kelime; unsur isminden sonra '{}'".format(k,l,item_3,unsur_referanslari_listesi [i],unsur_isimleri_listesi [i],sesli_harfler [n]) + ' sesli harfi aldığı için ünsüz yumuşaması gerekebilir.')
                            else:
                                continue
                        else:
                           continue
        ...
        for i in range (0,toplam_unsur_sayisi-1):
            for k in range (0, toplam_unsur_sayisi-1): 
                 for l, item_3 in enumerate(unsur_tanimlari_listesi_split_son [k]):
                    for j, item_2 in enumerate(unsuz_yumusamasi_harfleri_listesi_initial):
                        for n in range (0,8):
                           if unsur_tanimlari_listesi_split [k] [l].startswith (unsur_referanslari_listesi [i]): 
                              unsur_i_tanimi_her_bir_kelimenin_harf_listesi = list(unsur_tanimlari_listesi_split_son [k] [l])
                              if unsur_tanimlari_listesi_split [k] [l] [1] == sesli_harfler [n]:
                                  if unsur_isimleri_listesi [i].endswith (unsuz_yumusamasi_harfleri_listesi_initial [j]):  #[i] [-1]
                                     unsur_i_tanimi_her_bir_kelimenin_harf_listesi [0] = unsur_isimleri_listesi_unsuz_yumusamasi_hali [i]
                                     unsur_tanimlari_listesi_split_son [k] [l] = ''.join(unsur_i_tanimi_her_bir_kelimenin_harf_listesi)
                                 ...
                                 unsur_i_tanimi_her_bir_kelimenin_harf_listesi [0] = unsur_isimleri_listesi [i]
                                 unsur_tanimlari_listesi_split_son [k] [l] = ''.join(unsur_i_tanimi_her_bir_kelimenin_harf_listesi)
                                 #unsur_tanimlari_listesi_split_son [k] [l] = unsur_tanimlari_listesi_split_son [k] [l].replace (unsur_referanslari_listesi [i], unsur_isimleri_listesi [i]) #Yukarıdaki kod ile unsur referanslarını yerlerine yerleştirdik. Bir önceki kodda sadece unsuz yumusamasını değiştirdiğimiz için
    #-kalan tüm değişimleri bu kodlar ile gerçekleştirdik.
        ...
        '''
        for i, item in enumerate(unsur_isimleri_listesi):
            for k in range (0, len(unsur_tanimlari_listesi_split_son)):   ##(1,
                for l, item_3 in enumerate(unsur_tanimlari_listesi_split_son [k]):
        ...
        for i in range (0,toplam_unsur_sayisi):
            if i+2 < toplam_unsur_sayisi:
                istem_cümlesi [i]=  str(unsur_tanimlari_listesi_string [i]) + ' en az bir ' + str(unsurlar [i].get(isim)) + ' (' + str(((unsurlar [i].get(referans)).capitalize())) + '), '
