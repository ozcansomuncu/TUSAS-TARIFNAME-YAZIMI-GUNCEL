...
harici_unsur_tanimlari_listesi =["hava aracından hedefine taarruz etmek amacıyla gönderilen", "Ma basınç uygulayarak stabilitesini sağlayan", "teknisyen tarafından Pları döndürmek için kullanılan, Pa dönmesi için gerekli hareketi sağlayan"]
...
harici_unsurlar [0] = {
   referans: "M",
   isim: "mühimmat",
   tanim: harici_unsur_tanimlari_listesi [0]
   }

harici_unsurlar [1] = {
   referans: "P",
   isim: "pabuç",
   tanim: harici_unsur_tanimlari_listesi [1] 
   }

harici_unsurlar [2] = {
   referans: "A",
   isim: "alyan",
   tanim: harici_unsur_tanimlari_listesi [2]
   }
...
for z in range (0,toplam_harici_unsur_sayisi):            
   for j, item_2 in enumerate(unsuz_yumusamasi_harfleri_listesi_initial):
      for k in range (0, len(harici_unsur_tanimlari_listesi_split_son)):
         for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split_son [k]):
            for n in range (0,8):   #sesli_harflerin toplam adedinden dolayı 8.             
               if harici_unsurlar [z].get(isim) [-1] == (unsuz_yumusamasi_harfleri_listesi_initial [j]): #Buradan 3.(i=2) ve 5. (i=4) unsur isimleri geçecek
                  if harici_unsur_tanimlari_listesi_split [k] [l] [0] == (harici_unsurlar [z].get(referans)): # [k] [l] = [3] [5] ve [3] [9] geçti.
                     if harici_unsur_tanimlari_listesi_split [k] [l] [1] == sesli_harfler [n]:
                        harici_unsur_tanimlari_listesi_split_son [k] [l] = list(harici_unsur_tanimlari_listesi_split_son [k] [l])
                        harici_unsur_tanimlari_listesi_split_son [k] [l] [0] = unsur_isimleri_listesi_unsuz_yumusamasi_hali [z]
                        harici_unsur_tanimlari_listesi_split_son [k] [l] = "".join(harici_unsur_tanimlari_listesi_split_son [k] [l])
                     else:
                         continue
                  else: 
                     continue
...
for z in range (0,toplam_harici_unsur_sayisi):  
  for k in range (0, len(harici_unsur_tanimlari_listesi_split_son)):
     for l, item_3 in enumerate(harici_unsur_tanimlari_listesi_split_son [k]):
        if harici_unsur_tanimlari_listesi_split [k] [l] [0] == (harici_unsurlar [z].get(referans)): # [k] [l] = [3] [5] ve [3] [9] geçti.
           harici_unsur_tanimlari_listesi_split_son [k] [l] = list(harici_unsur_tanimlari_listesi_split_son [k] [l])
           harici_unsur_tanimlari_listesi_split_son [k] [l] [0] = harici_unsurlar [z].get(isim)
           harici_unsur_tanimlari_listesi_split_son [k] [l] = "".join(harici_unsur_tanimlari_listesi_split_son [k] [l])
        else:
           continue
...
for i in range (0,toplam_harici_unsur_sayisi):
   if i < toplam_harici_unsur_sayisi:
      istem_cümlesi [i]=  harici_unsur_tanimlari_listesi_string [i] + ' en az bir ' + harici_unsurlar [i].get(isim) + ' (' + str(harici_unsurlar [i].get(referans)) + '), '
   elif i == toplam_harici_unsur_sayisi:
      istem_cümlesi [i]=  harici_unsur_tanimlari_listesi_string [i] + ' en az bir ' + harici_unsurlar [i].get(isim) + ' (' + str(harici_unsurlar [i].get(referans)) + ') '
   else:
      break
